...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.cors().and().csrf().disable()
            ...
            .antMatchers(HttpMethod.GET, "/api/productImages/**").permitAll().and()
            .authorizeRequests().antMatchers(HttpMethod.GET,"/api/roles/**").hasAnyAuthority("ADMIN", "STAFF").and()
            .authorizeRequests().antMatchers(HttpMethod.POST, "/api/categories/**", "/api/products/**", "/api/productImages/**").hasAnyAuthority("ADMIN", "STAFF").and()
            .authorizeRequests().antMatchers(HttpMethod.POST,"/api/roles/**").hasAuthority("ADMIN").and()
            .authorizeRequests().antMatchers(HttpMethod.PUT, "/api/categories/**", "/api/productImages/**", "/api/bills/**", "/api/billDetails/**").hasAnyAuthority("ADMIN", "STAFF").and()
            .authorizeRequests().antMatchers(HttpMethod.PUT,"/api/roles/**").hasAuthority("ADMIN").and()
            .authorizeRequests().antMatchers(HttpMethod.DELETE, "/api/categories/**", "/api/products/**", "/api/productImages/**", "/api/users/**", "/api/bills/**", "/api/billDetails/**").hasAnyAuthority("ADMIN", "STAFF").and()
            .authorizeRequests().antMatchers(HttpMethod.DELETE,"/api/roles/**").hasAuthority("ADMIN").and()
